...
void vLogicTask(void *argument)
{
    ...
    HAL_GPIO_WritePin(GPIOD, SD_MODE_Pin, GPIO_PIN_RESET); //brakujacy SD_MODE
    HAL_GPIO_WritePin(GPIOA, MOTOR_EN_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOA, TMC_CS_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOB, SPI_EN_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOC, DIR_Pin, GPIO_PIN_RESET); // SD_MODE na 0 zeby uklad sluchal SPI

    printf("[LOGIC] oczekiwanie az spadnie ponizej 800mV\r\n");

    // dzielnik wymusza czekanie az kondensatory sie rozladuja i uklad sie zresetuje
    ...
    osDelay(2000); // chwila dla cyfrowki zeby wstala
    HAL_GPIO_WritePin(GPIOA, MOTOR_EN_Pin, GPIO_PIN_RESET);
    osDelay(500);

    // konfiguracja rejestrów przez oficjalne API
    printf("[LOGIC] konfiguracja rejestrow\r\n");

    tmc5160_writeRegister(0, TMC5160_GSTAT, 0x07);            // czyszczenie smieci z GSTAT po wlaczeniu zasilania
    tmc5160_writeRegister(0, TMC5160_SWMODE, 0x00);           // wylaczenie krancowek
    tmc5160_writeRegister(0, TMC5160_CHOPCONF, 0x000100C3);   // CHOPCONF przeklejony z PDF od TMC
    tmc5160_writeRegister(0, TMC5160_IHOLD_IRUN, 0x00061F0A); // IRUN na 31 (MAX)
    tmc5160_writeRegister(0, TMC5160_TPOWERDOWN, 0x0A);
    tmc5160_writeRegister(0, TMC5160_GCONF, 0x04);            // odpala StealthChop (cicha praca)
    tmc5160_writeRegister(0, TMC5160_TPWMTHRS, 0x1F4);

    // parametry rampy przeklejone 1:1 z dokumentacji producenta
    tmc5160_writeRegister(0, TMC5160_A1, 1000);
    tmc5160_writeRegister(0, TMC5160_V1, 50000);
    tmc5160_writeRegister(0, TMC5160_AMAX, 500);
    tmc5160_writeRegister(0, TMC5160_VMAX, 200000);
    tmc5160_writeRegister(0, TMC5160_DMAX, 700);
    tmc5160_writeRegister(0, TMC5160_D1, 1400);
    ...
    tmc5160_writeRegister(0, TMC5160_XACTUAL, 0); // XACTUAL na 0 - zeby nie myslal ze jest u celu juz

    osDelay(1000);

    // narazie testowy ping pong mozna by powiedziec wokol zera XD
    // potem tutaj bedzie sterowane po CANie z joysticka
    while(1)
    {
        printf("[RUNNING] Jazda na -51200\r\n");
        tmc5160_writeRegister(0, TMC5160_XTARGET, -51200);
        osDelay(4000);

        printf("[RUNNING] Powrot do zera\r\n");
        tmc5160_writeRegister(0, TMC5160_XTARGET, 0);
        osDelay(4000);
    }
}
